...
enum AkMemPoolAttributes
{
...
#if defined(AK_WIN)
	AkVirtualAlloc	= 2,	///< Block allocation type: Use AK::VirtualAllocHook() (AK_WIN & Xbox360 only)
	...
	AkVirtualAlloc	= 2,	///< Block allocation type: Use AK::VirtualAllocHook() (AK_WIN & Xbox360 only)
	AkPhysicalAlloc	= 4,	///< Block allocation type: Use AK::PhysicalAllocHook() (Xbox360 only)
	AkAllocMask		= AkNoAlloc | AkMalloc | AkVirtualAlloc | AkPhysicalAlloc,	///< Block allocation type mask (3 bits)
#elif defined (AK_PS3)
	AkAllocMask		= AkNoAlloc | AkMalloc,										///< Block allocation type mask (3 bits)
#elif defined(AK_WII)
	AkMallocMEM2	= 2,	///< Block allocation type: Use AK::AllocMEM2Hook() (Wii only)
	AkAllocMask		= AkNoAlloc | AkMalloc | AkMallocMEM2,						///< Block allocation type mask (3 bits)
#elif defined (AK_NGP)
	AkAllocMask		= AkNoAlloc | AkMalloc,										///< Block allocation type mask (3 bits)
#elif defined (AK_3DS)
	AkMallocDevice	= 2,		///< Block allocation type: Use AK::AllocDeviceHook() (3DS only)
	AkAllocMask = AkNoAlloc | AkMalloc | AkMallocDevice,					///< Block allocation type mask (3 bits)
#else
	#error AkMemoryMgr.h: Platform is not defined 
#endif
...
{   
	...
	namespace MemoryMgr
	{
		...
		AK_EXTERNAPIFUNC( AKRESULT, SetPoolName )( 
			AkMemPoolId     in_poolId,			///< ID of memory pool
			const char*		in_pszPoolName		///< Pointer to ansi name string
			);
		...
		/// \sa
		/// - \ref memorymanager
		AK_EXTERNAPIFUNC( wchar_t*, GetPoolName )( 
			AkMemPoolId     in_poolId			///< ID of memory pool
			);
