...
namespace AK
{
	...
	public:
		...
		/// Reads an ansi string into a fixed-size buffer. 
		/// \return	True if the operation was successful, False otherwise. An insufficient buffer size does not cause failure.
		bool ReadString( 
			char * out_pszString,	///< Pointer to a fixed-size buffer
			long in_nMax )			///< Maximum number of characters to be read in out_pszString, including the terminating NULL character
		{
			bool bRet = true;
			wchar_t wideString[ AK_MAX_PATH ];	
			if(AKPLATFORM::AkCharToWideChar( out_pszString, AK_MAX_PATH, wideString ) > 0)
				bRet = ReadString( wideString, in_nMax );
			return bRet;
		}
		//@}
	};
	...
	public:
		...
		{
			AkInt32 cChars = (long) wcslen( in_pszString );
			...
			return cWritten == (long)( cChars * sizeof( wchar_t ) );
		}
			...
			const char * in_pszString )	///< String to be written
		{
			bool bRet = true;
			wchar_t wideString[ AK_MAX_PATH ];	
			if(AKPLATFORM::AkCharToWideChar( in_pszString, AK_MAX_PATH, wideString ) > 0)
				bRet = WriteString( wideString );
			return bRet;
		}
		//@}
	};
